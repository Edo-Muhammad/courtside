                ...
                <tr>
                    <td>
                        ...
                        @elseif ($item->metode_pembayaran === 'qris')
                            <span class="badge bg-primary px-2 py-2 fs-6">QRIS</span>
                        @elseif ($item->metode_pembayaran === 'di_tempat')
                            <span class="badge bg-secondary px-2 py-2 fs-6">🏪 Di Tempat</span>
                        @else
                    ...
                    <td>
                        ...
                        $metodeClass = match($item->metode_pembayaran ?? '') {
                            'qris'      => 'bg-primary',
                            'di_tempat' => 'bg-secondary',
                            'transfer'  => 'bg-info text-dark',
                            default     => 'bg-light text-dark',
                        };
                        @endphp
                        <span class="badge {{ $metodeClass }}">{{ $item->metode_label }}</span>
                    </td>
                    <td>
                        @php
                    ...
                    <td>
                        ...
                        <form action="{{ route('admin.pembayaran.reject', $item) }}" method="POST" class="d-inline"
